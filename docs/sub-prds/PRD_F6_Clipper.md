...
| 项目 | 内容 |
|---|---|
| 文档类型 | 子 PRD（Feature PRD）— F6 Chrome 剪藏插件 |
| 版本 | v0.1 草案 |
| 日期 | 2026-07-26 |
| 上游文档 | 主 PRD《PrismDocs MVP 主产品需求文档》v0.1 §3-F6（关联 §3-F5、§3-F7、§5）；BRD v0.1 §6.4、§12-M3 |
...
**建议**：本地端口（127.0.0.1 随机高位端口 + 桌面端生成的配对 token，首次配对经用户在插件内粘贴/一键确认），因安装零成本契合"引流插件独立分发"（BRD M3）且规避 1MB 分片复杂度。**标注为开放决策 OQ-1（§10），技术设计阶段定案**；无论何种方案，均满足主 PRD §5"剪藏与文档不经过我方服务器"。
...
| # | 问题 | 倾向 |
|---|---|---|
| OQ-1 | 同步通道：native messaging vs 本地端口（REQ-6.5） | 倾向本地端口 + 配对 token；技术设计定案 |
