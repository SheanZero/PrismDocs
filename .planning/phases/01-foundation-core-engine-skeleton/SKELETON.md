...
| Module boundary | Cargo workspace: `prism-core` / `prism-store` / `prism-llm` / `prism-mcp`, with `src-tauri` (Cargo package **`prism-docs-shell`**, lib target `prism_docs_shell_lib` including `rlib`) as a thin command layer (D-01) | The boundary is enforced by the compiler rather than by convention. The shell's package name is fixed rather than left to the scaffolder's app-name default because shell integration tests are addressed as `cargo test -p prism-docs-shell`. Phase 3's anchoring engine must be drivable headless against an adversarial corpus, so no core crate may depend on `tauri`. Slots reserved for `prism-anchor` (Phase 3) and `prism-lens` (Phase 4). |
...
| Backup | Explicit export via SQLite `VACUUM INTO` plus a zip of the project data directory, carrying a self-identifying `manifest.json` (D-12) | In WAL mode a database is three files, so a copy of the main file silently loses uncheckpointed writes. `VACUUM INTO` produces one consistent standalone file. NFR-02's "single backup-able directory" is met by an export feature, not by moving the database into the repo. The archive names its own `project_id`, and project resolution adopts an unclaimed restored directory before minting a new id — so a restore works on a machine holding neither the `.prismdocs/` marker nor a registry row, and the restore promise does not depend on whether the marker is committed. |
